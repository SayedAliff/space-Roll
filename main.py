...
# https://github.com/kidscancode/gamedev
# See LICENSE.txt
import pygame as pg
import sys
from os import path
...
class Game:
    ...
    def draw_text(self, text, size, color, x, y, align='midtop'):
        ...
        font = pg.font.Font(path.join(FONT_NAME), size)
    ...
    def new(self):
        ...
        pg.mixer.music.load(path.join('SimpleBeat.ogg'))
    ...
    def load_data(self):
        ...
        self.player_light_img = pg.image.load(path.join('circle_1000.png')).convert_alpha()
        ...
        # spritesheets
        self.spritesheet = SpritesheetWithXML(path.join('sheet'))
        self.beam_sheet = SpritesheetWithXML(path.join('beams'))
        self.expl_sheet = SpritesheetWithXML(path.join('spritesheet_regularExplosion'))
        self.expl_player_sheet = SpritesheetWithXML(path.join('spritesheet_sonicExplosion'))
        self.ship_particle_img = pg.image.load(path.join(PLAYER_THRUST_IMG)).convert_alpha()
        # rock images - 4 sizes
        ...
        self.background = pg.image.load(path.join('starfield.png')).convert_alpha()
        ...
        # sounds
        self.shield_down_sound = pg.mixer.Sound(path.join(SHIELD_DOWN_SOUND))
        self.shield_down_sound.set_volume(1.0)
        self.alien_fire_sound = pg.mixer.Sound(path.join(ALIEN_BULLET_SOUND))
        self.alien_fire_sound.set_volume(1.0)
        self.hyper_sound = pg.mixer.Sound(path.join(HYPER_SOUND))
        self.bomb_tick_sound = pg.mixer.Sound(path.join(BOMB_TICK_SOUND))
        self.bomb_tick_sound.set_volume(0.5)
        self.bullet_sounds = []
        for sound in BULLET_SOUNDS:
            snd = pg.mixer.Sound(path.join(sound))
            snd.set_volume(0.5)
            self.bullet_sounds.append(snd)
        self.bomb_launch_sound = pg.mixer.Sound(path.join(BOMB_LAUNCH_SOUND))
        self.rock_exp_sounds = []
        for sound in ROCK_EXPL_SOUNDS:
            self.rock_exp_sounds.append(pg.mixer.Sound(path.join(sound)))
        self.bomb_exp_sounds = []
        for sound in BOMB_EXPL_SOUNDS:
            self.bomb_exp_sounds.append(pg.mixer.Sound(path.join(sound)))
        self.pow_sounds = {}
        for pow_type in POW_SOUNDS.keys():
            self.pow_sounds[pow_type] = pg.mixer.Sound(path.join(POW_SOUNDS[pow_type]))
